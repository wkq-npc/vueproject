<template>
  <div class="student">
    ...
    <ul>
      <li v-for="(item, index) in list" :key="index" @click="message(item.stu_id)">
        <p>
          {{ item.name }}
          <span v-show="isShow">&gt;</span>
          <span class="fuhao" v-show="!isShow"
            ><van-icon name="close" size="26" @click.stop="Dialog(index)"
          /></span>
        </p>
      </li>
    </ul>
...
export default {
  ...
  methods: {
    ...
    message(id) {
      this.$router.push("/studentmanage/" + id);
    },
    Dialog(index) {
      this.abc = index;
      this.$dialog
